...
# 2 less important features remove and train
train_df  = train.drop("Parch", axis=1)
test_df  = test.drop("Parch", axis=1)
train_df  = train.drop("CabinBool", axis=1)
test_df  = test.drop("CabinBool", axis=1)
from sklearn.model_selection import train_test_split
x_def = train_df.drop(['Survived', 'PassengerId'], axis=1)
test_def = test_df.drop(['PassengerId'], axis=1);
y_def = train_df["Survived"]
x_train_def, x_val_def, y_train_def, y_val_def = train_test_split(x_def, y_def, test_size = 0.22, random_state = 0)
# Random Forest
random_forest = RandomForestClassifier(n_estimators=100, oob_score = True)
random_forest.fit(x_train_def, y_train_def)
Y_prediction = random_forest.predict(x_val_def)
random_forest.score(x_train_def, y_train_def)
acc_random_forest = round(random_forest.score(x_train_def, y_train_def) * 100, 2)
